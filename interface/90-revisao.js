...
function renderRevisaoDeposito() {
  ...
  const painelDisputaSobraHtml = (() => {
    ...
    return `
      <div id="pcDisputaSobraOverlay" class="pc-overlay-fade" style="position:fixed; inset:0; z-index:100; background:rgba(8,9,10,.6); backdrop-filter:blur(6px); -webkit-backdrop-filter:blur(6px); display:flex; align-items:center; justify-content:center; padding:20px;">
...
async function renderQuadroMedias() {
  ...
  conteudo.innerHTML = `
    ...
    <div style="display:flex; align-items:center; gap:8px; background:var(--pc-glass); border:1px solid var(--pc-glass-border); border-radius:10px; padding:9px 11px; margin-bottom:14px;">
      ...
      <span style="font-size:11px; color:var(--pc-ink-dim); line-height:1.4;"><b style="color:var(--pc-ink);">${reaisNoEstado} pessoa${reaisNoEstado === 1 ? "" : "s"} ${reaisNoEstado === 1 ? "real" : "reais"}</b> já ${reaisNoEstado === 1 ? "depositou" : "depositaram"} em ${pcState.estado}. Enquanto não chegar em ${LIMIAR_MEDIANA_REAL}, a mediana é um ponto de partida — não a opinião do grupo.</span>
    </div>
